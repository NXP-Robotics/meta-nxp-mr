# Carried from meta-qt6 remotes/qt/6.4 branch to match Ubuntu 24.04 Qt 6.4.2 ABI.
# Do NOT include qt6-git.inc (has 6.9.3 SRCREVs); SRCREV hardcoded below.
...
# qt6.inc provides package splitting/naming — stable across Qt 6.x.
# Do NOT include qt6-git.inc (contains 6.9.3 SRCREVs).
include recipes-qt/qt6/qt6.inc

FILESEXTRAPATHS:prepend := "${THISDIR}/qtbase-6.4.3:"
...
PV     = "6.4.3"
SRCREV = "4822af4f41a8b39bf631f9f02e1a35f40a76ecc5"
...
# Generated .rcc/*.cpp files and qmake/qtpaths binaries embed TMPDIR — known Qt build artifact
...
# imx95-navq uses eglfs/wayland, not X11/xcb. Split the xcb QPA library into
# its own package so xcb/X11 deps don't block qtbase installation.
# ubuntu-base RCONFLICTS with libxcb-shape0 (ubuntu provides it via apt), so
# any package that depends on libxcb-shape would conflict with ubuntu-base.
# libQt6XcbQpa.so links xcb and pulls in those deps; isolating it removes the conflict.
PACKAGES =+ "${PN}-xcb-qpa"
FILES:${PN}-xcb-qpa = "${libdir}/libQt6XcbQpa.so.6* \
                       ${libdir}/libQt6XcbQpa.prl \
"
RRECOMMENDS:${PN}:remove = "${PN}-xcb-qpa"
ALLOW_EMPTY:${PN}-xcb-qpa = "0"
